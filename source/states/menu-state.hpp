...
// This struct is used to store the location and size of a button and the code it should execute
// when clicked
struct Button {
    // The position (of the top-left corner) of the button and its size in pixels
    glm::vec2 position, size;
    // The function that should be excuted when the button is clicked. It takes no arguments and
    // returns nothing.
    std::function<void()> action;

    // This function returns true if the given vector v is inside the button. Otherwise, false is
    // returned. This is used to check if the mouse is hovering over the button.
    bool isInside(const glm::vec2 &v) const {
        return position.x <= v.x && position.y <= v.y && v.x <= position.x + size.x &&
...
class Menustate : public our::State {
    ...
    // A material to be used to highlight hovered buttons (we will use blending to create a negative
    // effect).
    our::TintedMaterial *highlightMaterial;
    // A rectangle mesh on which the menu material will be drawn
    our::Mesh *rectangle;
    // A variable to record the time since the state is entered (it will be used for the fading
    // effect).
    float time;
    ...
    void onInitialize() override {
        ...
        // Create a material to highlight hovered buttons with a darkened version of the same color
        highlightMaterial = new our::TintedMaterial();
        // Use the tinted material shaders
        ...
        // Set the tint to a darker version of the original button color
        // You can later override this dynamically per button based on its base color
        highlightMaterial->tint = glm::vec4(0.0f, 0.0f, 0.0f, 0.5f); // 50% black tint

        // Enable blending to darken the color
        ...
        // - The capture list [] which is the variables that the lambda should remember because it
        // will use them during execution.
        //      We store [this] in the capture list since we will use it in the action.
        // - The argument list () which is the arguments that the lambda should receive when it is
        // called.
        //      We leave it empty since button actions receive no input.
        // - The body {} which contains the code to be executed.
        buttons[0].position = {318.0f, 580.0f};
        buttons[0].size = {298.0f, 123.0f};
        buttons[0].action = [this]() { this->getApp()->changeState("play"); };

        buttons[1].position = {663.0f, 580.0f};
        buttons[1].size = {298.0f, 123.0f};
    ...
    void onDraw(double deltaTime) override {
        ...
        // The projection matrix applys an orthographic projection whose size is the framebuffer
        // size in pixels so that the we can define our object locations and sizes in pixels. Note
        // that the top is at 0.0 and the bottom is at the framebuffer height. This allows us to
        // consider the top-left corner of the window to be the origin which makes dealing with the
        // mouse input easier.
        glm::mat4 VP = glm::ortho(0.0f, (float)size.x, (float)size.y, 0.0f, 1.0f, -1.0f);
        // The local to world (model) matrix of the background which is just a scaling matrix to
        // make the menu cover the whole window. Note that we defind the scale in pixels.
